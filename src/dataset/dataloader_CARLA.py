...
class CarlaEquirectangular(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        frame_path, range_path, label_path = self.data_path[idx]

        rgb_img = cv.imread(frame_path)

        labels = cv.imread(label_path)
        semantic_img = labels[...,2]
        ...
        semantics, cam_tensor = equirect2Fisheye_FOV(semantic_img, outShape,
                                    f=f,
                                    w_=d,
                                    angles=[angle1, angle2 ,angle3],
                                    interpolation = cv.INTER_NEAREST,
        ...
        return color_img, range_img, unit_vec, normals, semantics, endcoding

def main():
    data_path_train = [(bin_path, bin_path.replace("rgb", "range"), bin_path.replace("rgb", "labels"))  for bin_path in glob.glob(f"/home/appuser/mnt/CARLA360/dataset/val/rgb/*.png")]
    config = {}
    ...
    for batch_idx, (color_img, range_img, xyz, normals, semantics, encoding) in enumerate(dataloader_train):
